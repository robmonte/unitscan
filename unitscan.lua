...
function unitscan.editor()
	...
	editorFrame.Select:SetSize(90, 20)
	editorFrame.Select:SetPoint("TOP", 0, -6)
	editorFrame.Select:SetText("Highlight All")
...
function SlashCmdList.UNITSCAN(parameter)
	...
	elseif name == 'volume low' then
		volumeLevel = 'low'
		unitscan.print("alert volume now set to low")
	elseif name == 'volume high' then
		volumeLevel = 'high'
		unitscan.print("alert volume now set to high")
